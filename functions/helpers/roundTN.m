function [a,e]=roundTN(a,tol,method)
% a=roundTN(a,tol)
% ----------------
...
%
% tol 		=	two options: 
%               - scalar, relative approximation error
%               - array, round to specific rank
% method    =   - 'error' - round to relative error
%           =   - 'rank' - round to rank
%
% Reference
% ---------
...
for i=1:d-1
    ...
    elseif strcmp(method,'rank')
        ...
        error('Method has to be either "error" or "rank".')
    end
    e = sum(s(r+1:end));
    a.core{i}=reshape(U(:,1:r),[a.n(i,1),prod(a.n(i,2:end-1)),r]);
    % a.core{i}=reshape(U(:,1:r),[a.n(i,1),a.n(i,3),r]);
    a.n(i,end)=r;
    a.core{i+1}=reshape((S(1:r,1:r)*V(:,1:r)')*reshape(a.core{i+1},[a.n(i+1,1),prod(a.n(i+1,2:end))]),[r,a.n(i+1,3),a.n(i+1,end)]);   
    a.n(i+1,1)=r;
end
 a.core{d}=reshape(a.core{d},[a.n(d,1),a.n(d,3),a.n(d,4)]);
 a.norm = d;
end